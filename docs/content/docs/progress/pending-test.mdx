...
- Kling 视频：渠道调用格式新增 Kling，支持配置 Base URL、Access Key、Secret Key 和手动录入模型名；需验证配置保存、模型选择和渠道切换正确。
- Kling 视频：支持原生文生视频、首尾帧图生视频、JWT 鉴权和任务轮询，并提供标准/专业模式、16:9/9:16/1:1 比例及 5/10 秒设置；需使用真实 Kling 账号验证请求字段、状态查询、结果下载和浏览器 CORS。
